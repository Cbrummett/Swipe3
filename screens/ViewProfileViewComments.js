...
export default function ViewProfileViewComments({route, navigation}){
...
// loggedInUsers comment
//-----------------------------------------------------------------------------
    const OwnedComment = ({item}) => {
        return(
            <View style={styles.comment}>
                <Text style={styles.commentUsername}>{item.username}</Text>
                <Text style={styles.commentText}>  {item.commentText}</Text>
                <Pressable style={styles.smallButton} onPress={() => navigation.navigate('DeleteOwnComment', {
...
//-----------------------------------------------------------------------------
    const Item = ({item, loggedInUser}) => {
        if (loggedInUser.username == item.username ){
            return <OwnedComment item={item}/>;
        } else {
    ...
    return(
        <SafeAreaView style={{flex: 1}}>
            <View>
                ...
                <Pressable style={styles.button} onPress={() => navigation.replace('VisitProfile', {
                    loggedInUserID: loggedInUserID,
                    userID: userID
                    })} >
...
//-----------------------------------------------------------------------------
    function AddComment(postID, loggedInUser, addComment) {
        if (addComment === ""){
            ...
            setErrorMessage("");
            navigation.replace("ViewProfileViewComments", {
                loggedInUserID: loggedInUserID,
                postID: postID,
                userID: userID
            });
